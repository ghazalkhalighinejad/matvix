...
if __name__ == '__main__':
    ...
    parser.add_argument('--samples_path', type=str, default="biodeg/samples", help="path to samples json already extracted in previous step")
    ...
    if args.resume:
        if os.path.exists(f"biodeg/{save_path}"):
            articles_processed = [file.split(".")[0] for file in os.listdir(f"biodeg/{save_path}")]
    ...
    for article in process_bar:
        ...
        for i, prompt in enumerate(prompts):
            ...
            if not os.path.exists(f"biodeg/{save_path}/{article}/responses"):
                os.makedirs(f"biodeg/{save_path}/{article}/responses")
            
            if not os.path.exists(f"biodeg/{save_path}/{article}/prompts"):
                os.makedirs(f"biodeg/{save_path}/{article}/prompts")

            with open(f"biodeg/{save_path}/{article}/responses/response-{i}.txt", "w") as f:
                f.write(response)
            
            with open(f"biodeg/{save_path}/{article}/prompts/prompt-{i}.txt", "w") as f:
                f.write(text_query + "\n" + str(image_paths))

        if not os.path.exists(f"biodeg/{save_path}/{article}/merged"):
            os.makedirs(f"biodeg/{save_path}/{article}/merged")

        save_merged_given_prediction(f"biodeg/{save_path}/{article}/responses", f"biodeg/{save_path}/{article}/merged")